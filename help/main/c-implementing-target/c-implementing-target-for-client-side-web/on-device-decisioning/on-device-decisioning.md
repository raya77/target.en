...
![Server-side only flow diagram](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/server-side-only.png)
...
To learn more about which activities qualify for on-device decisioning, see [Supported features in on-device decisioning](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/supported-features.md). 
...
![On-device only flow diagram](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/on-device-only.png)
...
![On-device only flow diagram for subsequent page navigation and repeat visits](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/on-device-only-subsequent.png)
...
![Hybrid flow diagram for a first-time visitor](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/hybrid-first-time-visitor.png)
...
![Hybrid flow diagram for subsequent page navigation and repeat visits](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/hybrid-subsequent.png)
...
>[!NOTE]
>
>You must have the [!UICONTROL Admin] or [!UICONTROL Approver] [user role](/help/main/administrating-target/c-user-management/user-management.md) to enable or disable the On-Device Decisioning toggle.
...
1. Under **[!UICONTROL Account Details]**, slide the **[!UICONTROL On-Device Decisioning]** toggle to the "on" position.
   ...
   ![On-device decisioning toggle](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/on-device-decisioning-toggle.png)
...
After enabling the [!UICONTROL On-Device Decisioning] toggle, [!DNL Target] begins generating and propagating [rule artifacts](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/rule-artifact.md) for your client.
...
1. Under **[!UICONTROL Implementation Methods]** > **[!UICONTROL Main Implementation Method]**, click **[!UICONTROL Edit]** next to your at.js version (must be at.js 2.5.0 or later).
   ...
   ![Edit Main Implementation Method settings](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/main-implementation-method.png)
...
1. Select the desired decisioning method:
   ...
   * [!UICONTROL Hybrid]
   ...
   ![Edit at.js settings panel](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/global-settings.png)

### Global settings

You can configure a default [!UICONTROL Decisioning Method] for all of [!DNL Target] decisions. The various decisioning methods are [!UICONTROL Server-side only], [!UICONTROL On-device only], and [!UICONTROL Hybrid]. The decisioning method that is selected in the Target UI is configured in `window.targetGlobalSettings` under the `decisioningMethod` field. Learn more about the `decisioningMethod` in [targetGlobalSettings()](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/targetgobalsettings.md).
...
If you set the `decisioningMethod` in `window.targetGlobalSettings`, but would like to override the `decisioningMethod` for each Adobe Target decision according to your use case, you can do this procedure by specifying `decisioningMethod` in At.js2.5.0+’s [getOffers()](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/adobe-target-getoffers-atjs-2.md) call.
...
![On-Device Decisioning Eligible label on the activity's Overview page.](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/on-device-decisioning-eligible-label.png)
...
![On-Device Decisioning Eligible filter on Activities page.](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/assets/on-device-decisioning-filter.png)
...
1. Create and activate an [activity type that is supported by on-device decisioning](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/on-device-decisioning/supported-features.md), and verify that it is on-device decisioning eligible. 
